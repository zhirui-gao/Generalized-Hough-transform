...
def build_r_table(image, origin):
    ...
    '''
    edges = cv2.Canny(image, threshold1=MIN_CANNY_THRESHOLD,
...
def accumulate_gradients(r_table, grayImage):
    ...
    '''
    edges = cv2.Canny(grayImage, threshold1=MIN_CANNY_THRESHOLD,
                  threshold2=MAX_CANNY_THRESHOLD)
    gradient = gradient_orientation(edges)
...
def test():
    reference_image = cv2.imread("../images/1.png", 0)
    detect_s = general_hough_closure(reference_image)
    test_general_hough(detect_s, reference_image, "../images/1_test.png")
